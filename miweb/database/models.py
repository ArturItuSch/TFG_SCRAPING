...
class Partido(models.Model):
    ...
    ladoequipo2 = models.CharField(max_length=10, choices=[('Blue', 'Blue'), ('Red', 'Red')])
...
class SeleccionCampeon(models.Model):
    TIPO_SELECCION = [
        ('Pick', 'Pick'),
        ('Baneo', 'Baneo'),
    ]
    tipo = models.CharField(max_length=10, choices=TIPO_SELECCION)
    partido = models.ForeignKey(Partido, on_delete=models.CASCADE, related_name='selecciones')
    equipo = models.ForeignKey(Equipo, on_delete=models.CASCADE)
    campeon = models.ForeignKey(Campeon, on_delete=models.SET_NULL, null=True)
    orden = models.IntegerField()  # Orden dentro del draft
    lado = models.CharField(max_length=10, choices=[('Blue', 'Blue'), ('Red', 'Red')])

    def __str__(self):
        return f"{self.get_tipo_display()} {self.campeon} por {self.equipo} en {self.partido}"
